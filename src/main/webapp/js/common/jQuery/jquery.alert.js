(function($) {
	$.alerts = {
		...
		confirmOther : function(message, title, Btntext, callback) {
			if (title == null)
				title = '确认';
			if (Btntext != null)
				$.alerts.okOtherButton = Btntext;
			$.alerts._show(title, message, null, null, null, 'confirmOther',
					function(result) {
						if (callback)
							callback(result);
					});
		...
		_show : function(title, msg, value, width, height, type, callback) {
			...
			case 'iframe':
				...
				if ($.support
						&& ($.support == "6.0" || $.support == "7.0")) {
					$("#loading").css("fontSize", height * 0.9 + 'px');
				}
				;
				if ($.support && $.support == "6.0") {
		...
		_reposition : function() {
			...
			//document.body.scrollTop为后来所加，页面又滚动条显示时根据滚动条位置的变化显示 2016-02-23
			if (top == 0 & left == 0) {
				$("#popup_container").css({
					top : top + 200 + document.body.scrollTop + 'px',
					left : left + 380 + 'px'
				});
			} else {
				$("#popup_container").css({
	...
	jConfirmOther = function(message, title, Btntext, callback) {
		$.alerts.confirmOther(message, title, Btntext, callback);
	};
